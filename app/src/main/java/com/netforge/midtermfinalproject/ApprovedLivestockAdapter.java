...
public class ApprovedLivestockAdapter extends RecyclerView.Adapter<ApprovedLivestockAdapter.ApprovedLivestockViewHolder> {

    private List<Livestock> approvedLivestockRequests;
    private Set<String> selectedDocumentIds; // Tracks selected document IDs

    public ApprovedLivestockAdapter(List<Livestock> approvedLivestockRequests) {
        this.approvedLivestockRequests = approvedLivestockRequests;
        this.selectedDocumentIds = new HashSet<>(); // Initialize as an empty set
    }

    @NonNull
    @Override
    public ApprovedLivestockViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.activity_item_livestock_request, parent, false);
        return new ApprovedLivestockViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull ApprovedLivestockViewHolder holder, int position) {
        ...
        // Check if the CheckBox is null
        if (holder.selectCheckBox == null) {
            System.out.println("Adapter: CheckBox is null at position " + position);
            return;
        }
    ...
    public List<String> getSelectedDocumentIds() {
        return new ArrayList<>(selectedDocumentIds); // Convert the set to a list
    }

    // ViewHolder class
    public static class ApprovedLivestockViewHolder extends RecyclerView.ViewHolder {

        private TextView animalIDTextView, speciesBreedTextView, ageWeightTextView, healthStatusTextView;
        private CheckBox selectCheckBox;

        public ApprovedLivestockViewHolder(@NonNull View itemView) {
            super(itemView);
            ...
            selectCheckBox = itemView.findViewById(R.id.selectCheckBox); // Ensure this matches the layout
